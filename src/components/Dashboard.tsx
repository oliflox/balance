      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 420px), 1fr))', gap: 20, marginTop: 26, alignItems: 'start' }}>
        ...
        <div style={{ ...panel, gridColumn: 'span 2', minWidth: 0, animation: 'riseIn .6s ease both', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
            {/* Every label shares one grid cell, so the column measures itself
                against the widest ("6%" or "104.5 kg") with no hardcoded width.
                Each is then nudged down onto its own grid line. */}
            <div style={{ display: 'grid', flex: 'none', height: PLOT_H, pointerEvents: 'none' }}>
              {vm.chart.yLabels.map((l, i) => (
                <span
                  ...
                  style={{
                    ...
                    alignSelf: 'start',
                    justifySelf: 'end',
                    transform: `translateY(calc(${(grid[i].y / 330).toFixed(4)} * ${PLOT_H} - 50%))`,
                    fontSize: 10.5,
                    color: 'rgba(242,240,230,.35)',
            ...
            <div style={{ flex: 1, minWidth: 0, position: 'relative' }}>
              <svg viewBox="0 0 900 330" preserveAspectRatio="none" style={{ width: '100%', height: PLOT_H, display: 'block', overflow: 'visible' }}>
                {grid.map((g, i) => (
